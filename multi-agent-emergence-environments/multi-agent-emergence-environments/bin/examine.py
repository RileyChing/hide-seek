...
def main(argv):
    ...
    有关示例环境jsonnet，请参见
         mujoco-worldgen / examples / example_env_examine.jsonnet
    您可以在'examples'中找到保存的策略，以及在所受到训练的环境和使用的超参数。
    对于环境jsonnet文件，使用的命名为'examples / <env_name> .jsonnet'，
    对于策略权重文件（超参数），使用的命名为'examples / <env_name> .npz。
    ...
    if len(names) >= 2:  # run policies on the environment 在环境上运行策略
        ...
        # environments code.
        # 在此处导入PolicyViewer和load_policy
        # 因为它们取决于几个仅播放策略所需的软件包，而不是任何环境代码所需的软件包。
