...
class HideAndSeekRewardWrapper(gym.Wrapper):
    '''
        ...
        建立捉迷藏动态（请参阅下面的不同奖励类型）。
            除非基础环境指定“ n_hiders”和“ n_seekers”，
            否则默认情况下代理的前半部分为藏身者，后半部分为寻找者。
        参数：
            rew_type（字符串）：可以是
                'selfish'：藏身者和寻求者私自地玩耍。 如果搜索者可以看到任何隐藏者，则他们将得到1.0；
                    否则，将得到-1.0。 如果没有搜索者找到，藏身者将获得1.0，否则获得-1.0。
                'joint_mean'：藏匿者和搜索者团队获得的平均reward
                'joint_zero_sum'：仅当所有隐藏者都藏好时，隐藏者才得到1.0，否则为-1.0。
                     如果有任何搜索者看到隐藏者，则搜索者获得1.0。
            reward_scale（float）：按此比例缩放reward
    '''
    def __init__(self, env, n_hiders, n_seekers, rew_type='selfish', reward_scale=1.0):
        ...
        # 智能体名字用于在tensorboard上绘制智能体特定的奖励
...
class MaskUnseenAction(gym.Wrapper):
    '''
        Masks a (binary) action with some probability if agent or any of its teammates was being
        observed by opponents at any of the last n_latency time step
        ...
        *
        如果在最后n_latency时间步长的任何位置，智能体或其任何队友（x）被对手看到了，
        则以一定概率掩盖/屏蔽（二进制）动作

        参数：
             team_idx（int）：被掩盖/屏蔽动作的团队（x）的团队索引（例如：0 = 隐藏者）
             action_key（字符串）：要掩盖/屏蔽的动作key
    '''
